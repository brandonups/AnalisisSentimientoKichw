...
def leer():
    ...
    vneu = []
    vneg = []
    with open('diccionariokichwaespañol.csv', 'r', encoding = "utf-8") as archivo:
      fila = csv.DictReader(archivo, delimiter=',')
      for row in fila:
          if row['Etiqueta'] == 'bueno':
              vpos.append(row['Kichwa'])
          elif row['Etiqueta'] == 'neutro':
              vneu.append(row['Kichwa'])
          elif row['Etiqueta'] == 'malo':
              vneg.append(row['Kichwa'])
...
def metodo(frase):
    ...
    return json.dumps({'jaccard':'Positivo' if p == 0 else ( 'Neutro' if p == 1  else 'Negativo'),'coseno': 'Positivo' if temp == 0 else ( 'Neutro' if temp == 1  else 'Negativo')})
